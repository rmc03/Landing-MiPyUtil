...
- **Markup:** `<table>` semántico con `<th scope="col">`/`<th scope="row">` reales, nunca un grid de `div`, a partir de 40rem. Las cuatro columnas no caben por debajo de ~640px sin truncar valores ("Multiusuario", "Automática"), así que en móvil la comparación cambia de forma, no de contenido: una lista de capacidades (`<ul>` de `<li>`, cada una con su `<dl>` de tres pares fuente/veredicto) renderizada del mismo arreglo de datos, con la tabla oculta. El scroll horizontal con encabezado de fila fijo se probó y se descartó: obliga a desplazar para ver la columna del producto y aun así corta los valores largos. Lo que no cambia es que **los tres veredictos siguen presentes**: fusionar "Libreta" y "Excel" perdería distinciones reales (p. ej. "funciona en el apagón" es cierto para la libreta pero no para Excel, que necesita una PC encendida). Cada veredicto lleva su fuente escrita al lado (`dt`), no en una cabecera lejana que se pierde al desplazarse.
- **Estilo:** cuerpo en `plate` (nunca `board-panel`, para separarse de la banda que la contiene); el encabezado invierte a `board-panel` a propósito, como una placa remachada aparte, un tono más recesado que el cuerpo. La columna del producto lleva un lavado suave del acento sobre `plate` (`color-mix` ~16%, igual que Offline Badge) más una barra superior sólida de `accent` en su celda de encabezado; nunca relleno sólido en el cuerpo, para no introducir un segundo acento saturado. En la forma móvil las tres fuentes se apilan en una sola columna (fuente a la izquierda, veredicto a la derecha, un borde derecho común para los tres) y ese mismo lavado marca la tercera fila, la de MiPyUtil, sin borde de color: la distinguen el lavado, el peso y el tamaño, no un `border-left` teñido. El texto de esa columna es `ink` en el cuerpo (no `accent`: sobre el lavado no llega a 4.5:1 en tema claro) y `accent-deep` en el encabezado (sobre `board-panel` sin lavado, donde sí lo alcanza); `accent-deep` no solo sirve de hover, también es el tono de reserva del acento cuando este no tiene suficiente contraste por sí solo.
